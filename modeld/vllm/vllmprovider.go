...
func NewVLLMProvider(modelName string, backends []string, client *http.Client, caps modeld.CapabilityConfig, authToken string, tracker libtracker.ActivityTracker) modeld.Provider {
	if tracker == nil {
...
func (p *vLLMProvider) GetChatConnection(ctx context.Context, backendID string) (modeld.LLMChatClient, error) {
	if !p.CanChat() {
...
func (p *vLLMProvider) GetPromptConnection(ctx context.Context, backendID string) (modeld.LLMPromptExecClient, error) {
	if !p.CanPrompt() {
...
func (p *vLLMProvider) GetEmbedConnection(ctx context.Context, backendID string) (modeld.LLMEmbedClient, error) {
	return nil, fmt.Errorf("provider %s (model %s) does not support embeddings", p.GetID(), p.ModelName())
}

func (p *vLLMProvider) GetStreamConnection(ctx context.Context, backendID string) (modeld.LLMStreamClient, error) {
	if !p.CanStream() {
