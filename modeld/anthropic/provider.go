...
// NewAnthropicProvider returns a modeld.Provider for the direct Anthropic API.
func NewAnthropicProvider(apiKey, modelName string, backendURLs []string, capability modeld.CapabilityConfig, httpClient *http.Client, tracker libtracker.ActivityTracker) modeld.Provider {
	if httpClient == nil {
...
func (p *anthropicProvider) GetChatConnection(_ context.Context, _ string) (modeld.LLMChatClient, error) {
	if !p.CanChat() {
...
func (p *anthropicProvider) GetStreamConnection(_ context.Context, _ string) (modeld.LLMStreamClient, error) {
	if !p.CanStream() {
...
func (p *anthropicProvider) GetPromptConnection(_ context.Context, _ string) (modeld.LLMPromptExecClient, error) {
	if !p.CanPrompt() {
...
func (p *anthropicProvider) GetEmbedConnection(_ context.Context, _ string) (modeld.LLMEmbedClient, error) {
	return nil, fmt.Errorf("model %s (anthropic) does not support embeddings", p.modelName)
}

var _ modeld.Provider = (*anthropicProvider)(nil)
