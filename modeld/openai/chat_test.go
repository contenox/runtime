...
func TestUnit_GPT5Chat_SystemHoistedToInstructions(t *testing.T) {
	...
	_, err := client.Chat(context.Background(), []modeld.Message{
...
func TestUnit_GPT5Chat_ToolResultSentAsFunctionCallOutput(t *testing.T) {
	...
	_, err := client.Chat(context.Background(), []modeld.Message{
		...
		{Role: "assistant", ToolCalls: []modeld.ToolCall{
...
func TestUnit_GPT5Chat_ToolUsesResponsesFunctionShape(t *testing.T) {
	...
	_, err := client.Chat(
		context.Background(),
		[]modeld.Message{{Role: "user", Content: "read README"}},
		modeld.WithTools(modeld.Tool{
			Type: "function",
...
func TestUnit_GPT5Chat_ToolWithoutParametersSendsEmptyParametersObject(t *testing.T) {
	...
	_, err := client.Chat(
		context.Background(),
		[]modeld.Message{{Role: "user", Content: "run noop"}},
		modeld.WithTools(modeld.Tool{
			Type:     "function",
			Function: &modeld.FunctionTool{Name: "noop"},
		}),
	)
	require.NoError(t, err)
...
func TestUnit_GPT5Chat_VLLMChatCompletionsUnaffected(t *testing.T) {
	...
	_, err := client.Chat(context.Background(), []modeld.Message{{Role: "user", Content: "ping"}})
	require.NoError(t, err)
