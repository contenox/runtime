...
func TestSystemSearchThenResolveWithFiles(t *testing.T) {
	...
	t.Run("ContextualSearch", func(t *testing.T) {
		tests := []struct {
			name         string
			query        string
			wantContains string
		...
		}{
			{
				...
				wantContains: "finance, healthcare",
				notContains:  "hiking trip",
			},
			{
				name:         "Narrow search with parameters",
				query:        "args: top_k=1, epsilon=0.8 radius=20 | what are learning techniques",
				wantContains: "reinforcement learning",
				notContains:  "machine learning",
			},
			{
				name:         "Non-AI content exclusion",
				query:        "args: top_k=1, epsilon=0.8 radius=20 | outdoor activities",
				wantContains: "hiking trip",
				notContains:  "artificial intelligence",
			},
		}
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				require.Contains(t, content, tt.wantContains)
				if tt.notContains != "" {
					require.NotContains(t, content, tt.notContains)
				}
			})
		}
	})
