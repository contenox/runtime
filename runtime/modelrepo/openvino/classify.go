...
func classifyChatContext(messages []modelrepo.Message, toolsJSON string) classifiedChatContext {
	return classifyChatContextWithIdentity(messages, toolsJSON, contextasm.ManifestIdentity{})
}

func classifyChatContextWithIdentity(messages []modelrepo.Message, toolsJSON string, identity contextasm.ManifestIdentity) classifiedChatContext {
	ordered := make([]modelrepo.Message, 0, len(messages))
...
func messageSegmentContent(m modelrepo.Message) string {
