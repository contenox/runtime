...
func TestUnit_GeminiChat_ThinkingOnlyResponseIsNotAnError(t *testing.T) {
	...
	res, err := newTestChatClient(srv).Chat(context.Background(), []modeld.Message{{Role: "user", Content: "hi"}})
...
func TestUnit_GeminiChat_ClampsMaxOutputTokens(t *testing.T) {
	...
	_, err := client.Chat(context.Background(),
		[]modeld.Message{{Role: "user", Content: "hi"}},
		modeld.WithMaxTokens(999),
	)
	require.NoError(t, err)
...
func TestUnit_GeminiChat_BlockedPromptStillErrors(t *testing.T) {
	...
	_, err := newTestChatClient(srv).Chat(context.Background(), []modeld.Message{{Role: "user", Content: "hi"}})
...
func TestUnit_BuildGeminiRequest_MapsThinkingConfig(t *testing.T) {
	...
	req, err := buildGeminiRequest("gemini-2.5-pro", msgs, nil, []modeld.ChatArgument{modeld.WithThink("medium")})
	require.NoError(t, err)
	...
	req, err = buildGeminiRequest("gemini-3-flash", msgs, nil, []modeld.ChatArgument{modeld.WithThink("off")})
	require.NoError(t, err)
	require.NotNil(t, req.GenerationConfig.ThinkingConfig)
	...
	req, err = buildGeminiRequest("gemini-3-pro", msgs, nil, []modeld.ChatArgument{modeld.WithThink("auto")})
	require.NoError(t, err)
	require.Nil(t, req.GenerationConfig.ThinkingConfig)

	req, err = buildGeminiRequest("gemini-2.5-pro", msgs, nil, []modeld.ChatArgument{modeld.WithThink("medium")}, false)
	require.NoError(t, err)
