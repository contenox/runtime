...
func TestSystem_VLLM_Smoke(t *testing.T) {
	...
	apiBase, _, cleanup, err := modeld.SetupVLLMLocalInstance(ctx, model, tag, "none")
	...
	tests := []struct {
		...
		testFunc func(t *testing.T, provider modeld.Provider, apiBase string)
	}{
		{
			name: "basic_chat",
			caps: modeld.CapabilityConfig{
		...
		},
		{
			...
			testFunc: func(t *testing.T, provider modeld.Provider, apiBase string) {
				...
				resp, err := chatClient.Chat(ctx, messages,
					modeld.WithTemperature(0.1), // Very deterministic
					modeld.WithMaxTokens(30),
				)
		...
		},
		{
			...
			testFunc: func(t *testing.T, provider modeld.Provider, apiBase string) {
				...
				start := time.Now()
				stream, err := streamClient.Stream(ctx, []modeld.Message{{Role: "user", Content: prompt}})
		...
		},
		{
			...
			testFunc: func(t *testing.T, provider modeld.Provider, apiBase string) {
				...
				resp1, err := chatClient.Chat(ctx, messages,
					modeld.WithSeed(123),
					modeld.WithTemperature(0.7), // Keep some randomness but controlled
					modeld.WithMaxTokens(10),
				)
