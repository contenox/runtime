...
func (c *GeminiStreamClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
		...
		for sc.Scan() {
			...
			if len(chunk.Candidates) > 0 && len(chunk.Candidates[0].Content.Parts) > 0 {
				...
				if outText != "" || thinkingText != "" {
					select {
					case parcels <- &modeld.StreamParcel{
						Data:     outText,
...
var _ modeld.LLMStreamClient = (*GeminiStreamClient)(nil)
