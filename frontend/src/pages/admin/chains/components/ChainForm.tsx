...
export default function ChainForm({ chain }: ChainFormProps = {}) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const chainData: ChainDefinition = {
      ...
      description,
      tasks: parsedTasks,
      token_limit: chain?.token_limit || 4096, // Default token limit
      routing_strategy: chain?.routing_strategy || 'default',
    };

    if (isEditing) {
      updateChain.mutate(chainData);
    } else {
      createChain.mutate(chainData);
    }
  };
