...
export type TaskEventViewState = {
  ...
  /** From chain_started (local chain path or chain id). */
...
export function formatTaskOutput(output: unknown): string {
  ...
  if (output && typeof output === 'object' && Array.isArray((output as { messages?: unknown }).messages)) {
    const messages = (output as { messages: Array<Record<string, unknown>> }).messages;
    const assistant = [...messages].reverse().find(message => message.role === 'assistant');
    const last = assistant ?? messages[messages.length - 1];
