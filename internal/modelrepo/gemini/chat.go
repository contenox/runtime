...
func (c *GeminiChatClient) Chat(ctx context.Context, messages []modelrepo.Message, args ...modelrepo.ChatArgument) (modelrepo.Message, error) {
	...
	if len(response.Candidates) == 0 {
		return modelrepo.Message{}, fmt.Errorf("no candidates returned from Gemini for model %s", c.modelName)
	}
	...
	if len(candidate.Content.Parts) == 0 || candidate.Content.Parts[0].Text == "" {
		...
		return modelrepo.Message{}, fmt.Errorf("empty content from model %s", c.modelName)
	}

	return modelrepo.Message{Role: "assistant", Content: candidate.Content.Parts[0].Text}, nil
}
