...
<body>
  ...
  <script nonce="${nonce}">
    ...
    provider.addEventListener("change", () => vscode.postMessage({ type: "setProvider", value: provider.value }));
    model.addEventListener("change", () => vscode.postMessage({ type: "setModel", value: model.value }));
    think.addEventListener("change", () => vscode.postMessage({ type: "setThink", value: think.value }));
    hitl.addEventListener("change", () => vscode.postMessage({ type: "setHitl", value: hitl.value }));
    ...
    function render(state) {
      ...
      status.textContent = "Runtime settings are applied immediately.";
    }
