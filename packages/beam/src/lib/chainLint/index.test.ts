...
describe('lintChain', () => {
  ...
  it('warns when local_shell is exposed without allowlist', () => {
    const t = chatTask();
    t.execute_config!.tools_policies = {
      ...t.execute_config!.tools_policies,
      local_shell: {},
    };
  ...
  it('warns when wildcard tools expose local_shell without allowlist', () => {
    const t = chatTask({ id: 'wild' });
    t.execute_config!.tools = ['*'];
    t.execute_config!.tools_policies = {}; // wipe both policies
    const result = lintChain(chain(t));
  ...
  it('does NOT warn when local_shell is excluded via negation', () => {
    ...
    t.execute_config!.tools = ['*', '!local_shell'];
    delete t.execute_config!.tools_policies!.local_shell;
    const result = lintChain(chain(t));
    expect(
  ...
  it('does NOT warn when tools list is empty (no exposure)', () => {
  ...
  it('treats absent tools as exposing no registry tools', () => {
    const t = chatTask({ id: 'absent' });
    delete t.execute_config!.tools;
    t.execute_config!.tools_policies = {};
    const result = lintChain(chain(t));
    expect(
      result.find((d) => d.ruleId === 'hook_policies_missing_for_local_shell'),
...
describe('diagnosticsByTask', () => {
  it('groups by taskId', () => {
    ...
    b.execute_config!.tools_policies = { local_shell: {} };
