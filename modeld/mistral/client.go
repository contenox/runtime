...
func (c *mistralChatClient) Chat(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (modeld.ChatResult, error) {
...
func (c *mistralStreamClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
	cfg := chatConfigFromArgs(args)
	req, nameMap := chatcompletions.Build(c.modelName, messages, cfg)
	req.MaxTokens = modeld.ClampMaxOutputTokensPtr(req.MaxTokens, c.maxOutputTokens)
	req.Stream = true
	...
	go func() {
		...
		if err := sc.Err(); err != nil && err != io.EOF {
			reportErr(err)
			select {
			case parcels <- &modeld.StreamParcel{Error: fmt.Errorf("mistral: stream read: %w", err)}:
			case <-ctx.Done():
			}
			return
		}
...
var (
	_ modeld.LLMChatClient       = (*mistralChatClient)(nil)
	_ modeld.LLMStreamClient     = (*mistralStreamClient)(nil)
	_ modeld.LLMPromptExecClient = (*mistralPromptClient)(nil)
)
