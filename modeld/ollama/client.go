...
func buildOllamaOptions(config *modeld.ChatConfig, maxOutputTokens int) map[string]any {
	opts := make(map[string]any)
	if config.Temperature != nil {
		opts["temperature"] = *config.Temperature
	}
	if config.MaxTokens != nil {
		effective, _ := modeld.ClampMaxOutputTokens(*config.MaxTokens, maxOutputTokens)
...
func buildOllamaThink(config *modeld.ChatConfig) *api.ThinkValue {
...
func buildOllamaTools(config *modeld.ChatConfig) (api.Tools, error) {
