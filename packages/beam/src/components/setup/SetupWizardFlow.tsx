...
import { isCloudProviderType } from '../../lib/providerCatalog';
...
function providerKind(provider: string): 'cloud' | 'local' {
  const p = provider.trim().toLowerCase();
  if (p === 'vertex') return 'cloud';
  if (isCloudProviderType(p) && p !== 'ollama') return 'cloud';
  return 'local';
}
...
export function SetupWizardFlow({ variant }: SetupWizardFlowProps) {
  ...
  const saveAndContinue = (e: React.FormEvent) => {
    e.preventDefault();
    putConfig.reset();
    const body: { 'default-model'?: string; 'default-provider'?: string } = {};
    if (model.trim()) body['default-model'] = model.trim();
    if (provider.trim()) body['default-provider'] = provider.trim();
    if (!body['default-model'] && !body['default-provider']) return;
  ...
  const backendsTabLink = kind === 'cloud' ? '/backends?tab=cloud-providers' : '/backends?tab=backends';
  ...
  const refreshStatus = () => {
    void api.refreshSetupStatus()
      .then(status => queryClient.setQueryData(setupKeys.status(), status))
      .catch(() => queryClient.invalidateQueries({ queryKey: setupKeys.status() }));
  };
            ...
            <>
              ...
              {showDefaultsForm ? (
                <form
                  ...
                  {putConfig.isSuccess ? (
                    <P className="text-text-muted text-sm">{t('setup.saved')}</P>
                  ) : null}
                </form>
              ) : (
                <P className="text-text-muted text-sm">{t('setup.wizard.defaults_done_hint')}</P>
              )}
            </>
          )}
          ...
          {currentStep === 1 && (
            <>
              ...
              {registrationCommands.length > 0 ? (
                <Accordion title={t('setup.wizard.cli_advanced')} className="max-w-xl">
                  <div className="space-y-2">
                    {registrationCommands.map(issue => (
                      <P key={issue.code} className="text-text-muted font-mono text-xs">
                        {issue.cliCommand}
                      </P>
                    ))}
                  </div>
          ...
          {currentStep === 2 && (
            <>
              {healthIssues.length > 0 ? (
                <ul className="text-error list-inside list-disc space-y-1 text-sm">
              ...
              healthIssues.length === 0 ? (
                <P className="text-text-muted text-sm">{t('setup.wizard.health_pending')}</P>
              ) : null}
              {data.backendCount > 0 &&
              data.reachableBackendCount > 0 &&
              healthIssues.length === 0 ? (
  ...
  if (variant === 'page') {
    return (
      <Panel variant="surface">
        ...
        {progressAndBody}
        <footer className="border-surface-200 dark:border-dark-surface-600 border-t pt-4">
          {footerActions}
        </footer>
      </Panel>
    );
