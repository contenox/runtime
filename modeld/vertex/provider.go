...
// NewVertexProvider returns a modeld.Provider for a Vertex AI model.
// credJSON is the service account key JSON; empty string falls back to ADC.
func NewVertexProvider(publisher, modelName string, baseURLs []string, cap modeld.CapabilityConfig, credJSON string, httpClient *http.Client, tracker libtracker.ActivityTracker) modeld.Provider {
	if httpClient == nil {
...
func (p *vertexProvider) GetChatConnection(_ context.Context, _ string) (modeld.LLMChatClient, error) {
	if !p.CanChat() {
...
func (p *vertexProvider) GetPromptConnection(_ context.Context, _ string) (modeld.LLMPromptExecClient, error) {
	if !p.CanPrompt() {
...
func (p *vertexProvider) GetStreamConnection(_ context.Context, _ string) (modeld.LLMStreamClient, error) {
	if !p.CanStream() {
...
func (p *vertexProvider) GetEmbedConnection(_ context.Context, _ string) (modeld.LLMEmbedClient, error) {
	return nil, fmt.Errorf("model %s (vertex-%s) does not support embeddings", p.modelName, p.publisher)
}

var _ modeld.Provider = (*vertexProvider)(nil)
