...
func (d *activityTrackerDecorator) Create(ctx context.Context, principal string, req CreateRequest) (*CreateResponse, error) {
	reportErr, reportChange, end := d.tracker.Start(ctx, "create", "terminal_session", "cols", req.Cols, "rows", req.Rows, "cwd", req.CWD)
	defer end()
...
func (d *activityTrackerDecorator) UpdateGeometry(ctx context.Context, principal, id string, cols, rows int) error {
	reportErr, reportChange, end := d.tracker.Start(ctx, "update", "terminal_session", "sessionID", id, "cols", cols, "rows", rows)
	defer end()
