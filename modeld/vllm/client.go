...
type chatRequest struct {
	...
	Messages           []modeld.Message `json:"messages"`
	Temperature        *float64            `json:"temperature,omitempty"`
	MaxTokens          *int                `json:"max_tokens,omitempty"`
	TopP               *float64            `json:"top_p,omitempty"`
	Seed               *int                `json:"seed,omitempty"`
	Stream             bool                `json:"stream,omitempty"`
	Tools              []modeld.Tool    `json:"tools,omitempty"`
...
func convertChatToolCalls(toolCalls []chatToolCall) []modeld.ToolCall {
	out := make([]modeld.ToolCall, 0, len(toolCalls))
	for _, tc := range toolCalls {
		out = append(out, modeld.ToolCall{
			ID:   tc.ID,
			Type: tc.Type,
			Function: struct {
				Name      string `json:"name"`
...
func buildChatRequest(modelName string, messages []modeld.Message, args []modeld.ChatArgument, canThink ...bool) chatRequest {
	config := &modeld.ChatConfig{}
...
func buildChatRequestFromConfig(modelName string, messages []modeld.Message, config *modeld.ChatConfig, canThink ...bool) chatRequest {
	req := chatRequest{
		Model:       modelName,
		Messages:    messages,
