...
// openvinoProvider implements modeld.Provider. It mirrors the local provider:
...
func newProvider(name, modelDir string, caps modeld.CapabilityConfig, tracker libtracker.ActivityTracker) modeld.Provider {
	if tracker == nil {
...
func (p *openvinoProvider) GetChatConnection(ctx context.Context, _ string) (modeld.LLMChatClient, error) {
	if !p.CanChat() {
...
func (p *openvinoProvider) GetStreamConnection(ctx context.Context, _ string) (modeld.LLMStreamClient, error) {
	if !p.CanStream() {
...
func (p *openvinoProvider) GetPromptConnection(ctx context.Context, _ string) (modeld.LLMPromptExecClient, error) {
	if !p.CanPrompt() {
...
func (p *openvinoProvider) GetEmbedConnection(_ context.Context, _ string) (modeld.LLMEmbedClient, error) {
...
var _ modeld.Provider = (*openvinoProvider)(nil)
