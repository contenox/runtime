...
func TestManagerSystem(t *testing.T) {
	...
	t.Run("ChatExec_runs_llm_with_chat_history_and_returns_response", func(t *testing.T) {
		...
		resp, inputTokens, outputTokens, _, err := manager.ChatExec(ctx, history, 4096, []string{"ollama"}, "smollm2:135m")
		require.NoError(t, err)
	...
	t.Run("ChatExec_fails_when_no_messages_given", func(t *testing.T) {
		resp, _, _, _, err := manager.ChatExec(ctx, nil, 4096, []string{"ollama"}, "smollm2:135m")
		require.Error(t, err)
		require.Nil(t, resp)
	...
	t.Run("ChatExec_fails_when_last_message_is_not_user", func(t *testing.T) {
		...
		resp, _, _, _, err := manager.ChatExec(ctx, history, 4096, []string{"ollama"}, "smollm2:135m")
		require.Error(t, err)
		require.Nil(t, resp)
