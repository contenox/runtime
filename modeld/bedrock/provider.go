...
// NewBedrockProvider returns a modeld.Provider for an AWS Bedrock model via
// the Converse API. credBlob is optional static-credentials JSON; empty falls
// back to the ambient AWS credential chain.
func NewBedrockProvider(region, credBlob, modelName string, cap modeld.CapabilityConfig, httpClient *http.Client, tracker libtracker.ActivityTracker) modeld.Provider {
	if tracker == nil {
...
func (p *bedrockProvider) GetChatConnection(ctx context.Context, _ string) (modeld.LLMChatClient, error) {
	if !p.CanChat() {
...
func (p *bedrockProvider) GetStreamConnection(ctx context.Context, _ string) (modeld.LLMStreamClient, error) {
	if !p.CanStream() {
...
func (p *bedrockProvider) GetPromptConnection(ctx context.Context, _ string) (modeld.LLMPromptExecClient, error) {
	if !p.CanPrompt() {
...
func (p *bedrockProvider) GetEmbedConnection(_ context.Context, _ string) (modeld.LLMEmbedClient, error) {
	return nil, fmt.Errorf("model %s (bedrock) does not support embeddings via Converse", p.modelName)
}

var _ modeld.Provider = (*bedrockProvider)(nil)
