...
func TestUnit_Build_ToolUseAndToolResultRoundTrip(t *testing.T) {
	cfg := &modeld.ChatConfig{
		Tools: []modeld.Tool{{
			Type: "function",
			Function: &modeld.FunctionTool{Name: "fs.list", Description: "list files"},
		}},
	}
	msgs := []modeld.Message{
		{Role: "user", Content: "list /tmp"},
		{Role: "assistant", ToolCalls: []modeld.ToolCall{tc("toolu_1", "fs.list", `{"path":"/tmp"}`)}},
...
func TestUnit_DecodeResponse_RefusalReturnsErrRefused(t *testing.T) {
	...
	if !errors.Is(err, modeld.ErrRefused) {
