...
// Stream implements modeld.LLMStreamClient against the Gemini
// streamGenerateContent wire format (vertex-google).
func (c *vertexStreamClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
	...
	if request.GenerationConfig != nil {
		request.GenerationConfig.MaxOutputTokens = modeld.ClampMaxOutputTokensPtr(request.GenerationConfig.MaxOutputTokens, c.maxOutputTokens)
	}
	...
	go func() {
		...
		if err != nil {
			parcels <- &modeld.StreamParcel{Error: fmt.Errorf("failed to marshal stream request: %w", err)}
			return
		}
		...
		if err != nil {
			...
			reportErr(err)
			parcels <- &modeld.StreamParcel{Error: err}
			return
		}
		...
		for sc.Scan() {
			...
			if len(chunk.Candidates) > 0 && len(chunk.Candidates[0].Content.Parts) > 0 {
				...
				if outText != "" || thinkingText != "" {
					...
					select {
					case parcels <- &modeld.StreamParcel{Data: outText, Thinking: thinkingText}:
					case <-ctx.Done():
						return
					}
				}
		...
		if err := sc.Err(); err != nil && err != io.EOF {
			...
			reportErr(err)
			select {
			case parcels <- &modeld.StreamParcel{Error: err}:
			case <-ctx.Done():
			}
			return
		}
...
var _ modeld.LLMStreamClient = (*vertexStreamClient)(nil)
