...
func TestUnit_OllamaProvider_DoesNotInferThinkingFromModelName(t *testing.T) {
	provider := NewOllamaProvider("qwen3:8b", []string{"http://localhost:11434"}, nil, modeld.CapabilityConfig{}, "", nil)
	if provider.CanThink() {
		t.Fatalf("expected qwen3 model name alone not to set CanThink")
	}

	provider = NewOllamaProvider("custom", []string{"http://localhost:11434"}, nil, modeld.CapabilityConfig{CanThink: true}, "", nil)
	if !provider.CanThink() {
...
func TestUnit_OllamaChat_OmitsThinkWhenCapabilityIsFalse(t *testing.T) {
	...
	provider := NewOllamaProvider("qwen3:8b", []string{srv.URL}, srv.Client(), modeld.CapabilityConfig{CanChat: true}, "", nil)
...
func TestUnit_BuildOllamaThink_NormalizesLevels(t *testing.T) {
	...
	modeld.WithThink("auto").Apply(cfg)
	if got := buildOllamaThink(cfg); got != nil {
	...
	cfg = &modeld.ChatConfig{}
	modeld.WithThink("minimal").Apply(cfg)
	got = buildOllamaThink(cfg)
...
func TestUnit_BuildOllamaOptions_ClampsPositiveNumPredictOnly(t *testing.T) {
	cfg := &modeld.ChatConfig{}
	modeld.WithMaxTokens(9000).Apply(cfg)
	...
	negative := -1
	cfg = &modeld.ChatConfig{MaxTokens: &negative}
	opts = buildOllamaOptions(cfg, 2048)
