...
func (c *OllamaStreamClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
	...
	ch := make(chan *modeld.StreamParcel)
	go func() {
		defer close(ch)
		defer end()
		...
		if err != nil {
			reportErr(err)
			select {
			case ch <- &modeld.StreamParcel{Error: fmt.Errorf("ollama stream request failed for model %s: %w", c.modelName, err)}:
			case <-ctx.Done():
			}
			return
		}
...
var _ modeld.LLMStreamClient = (*OllamaStreamClient)(nil)
