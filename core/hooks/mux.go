...
func (m *Mux) Exec(ctx context.Context, startTime time.Time, input any, dataType taskengine.DataType, transition string, hookCall *taskengine.HookCall) (any, taskengine.DataType, string, error) {
	...
	case taskengine.DataTypeString:
		s, ok := input.(string)
		if !ok {
			return nil, taskengine.DataTypeAny, "",
	...
	if !strings.HasPrefix(trimmedInput, "/") {
		return input, dataType, transition, nil
	}
	...
	result, resultType, transition, err := hook.Exec(
		ctx, startTime, args, taskengine.DataTypeString, command, hookCall,
	)

	if err != nil {
		return result, resultType, transition, err
	}
	...
	if chatHist != nil {
		...
		return result, taskengine.DataTypeAny, transition,
			fmt.Errorf("hook result not string: %T", result)
	}

	return result, resultType, transition, nil
}
...
func (m *Mux) Get(name string) (func(context.Context, time.Time, any, taskengine.DataType, string, *taskengine.HookCall) (any, taskengine.DataType, string, error), error) {
