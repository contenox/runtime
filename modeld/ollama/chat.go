...
func (c *OllamaChatClient) Chat(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (modeld.ChatResult, error) {
	...
	if err != nil {
		reportErr(err)
		return modeld.ChatResult{}, fmt.Errorf("ollama API chat request failed for model %s: %w", c.modelName, err)
	}
	...
	// Convert Ollama tool calls → modeld.ToolCall
	// Ollama ToolCall:
...
var _ modeld.LLMChatClient = (*OllamaChatClient)(nil)
