...
func (s *store) Insert(ctx context.Context, sess *Session) error {
	...
	sess.WorkspaceID = s.workspaceID
...
func (s *store) GetByID(ctx context.Context, id string) (*Session, error) {
	return s.getByCondition(ctx, "id = $1 AND workspace_id = $2", id, s.workspaceID)
}

func (s *store) GetByIDAndPrincipal(ctx context.Context, id, principal string) (*Session, error) {
	return s.getByCondition(ctx, "id = $1 AND principal = $2 AND workspace_id = $3", id, principal, s.workspaceID)
}
...
func (s *store) ListByPrincipal(ctx context.Context, principal string, createdAtCursor *time.Time, limit int) ([]*Session, error) {
	...
	rows, err := s.Exec.QueryContext(ctx, `
		SELECT id, principal, cwd, shell, cols, rows, status, node_instance_id, workspace_id, created_at, updated_at
		FROM terminal_sessions
		WHERE principal = $1 AND status = $2 AND workspace_id = $3 AND created_at < $4
		ORDER BY created_at DESC, id DESC
		LIMIT $5`,
		principal, string(SessionStatusActive), s.workspaceID, cursor, limit,
	)
	if err != nil {
...
func (s *store) UpdateGeometry(ctx context.Context, id string, cols, rows int) error {
	...
	res, err := s.Exec.ExecContext(ctx, `
		UPDATE terminal_sessions SET cols = $1, rows = $2, updated_at = $3
		WHERE id = $4 AND status = $5 AND workspace_id = $6`,
		cols, rows, now, id, string(SessionStatusActive), s.workspaceID,
	)
	if err != nil {
...
func (s *store) Delete(ctx context.Context, id string) error {
	res, err := s.Exec.ExecContext(ctx, `DELETE FROM terminal_sessions WHERE id = $1 AND workspace_id = $2`, id, s.workspaceID)
	if err != nil {
...
func (s *store) DeleteByNodeInstanceID(ctx context.Context, nodeInstanceID string) error {
	_, err := s.Exec.ExecContext(ctx, `DELETE FROM terminal_sessions WHERE node_instance_id = $1 AND workspace_id = $2`, nodeInstanceID, s.workspaceID)
	if err != nil {
