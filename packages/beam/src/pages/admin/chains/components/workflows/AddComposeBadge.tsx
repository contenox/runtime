...
export default function ComposeEdgeBadge({
  ...
  // Map strategy to neutral token colors (CSS variables from @contenox/ui).
  const colorFor = useCallback((s?: string | null) => {
    ...
    switch ((s || 'default').toLowerCase()) {
      case 'override':
        return { fill: get('--color-primary-500') ?? '#525252', stroke: get('--color-primary-900') ?? '#0a0a0a' };
      case 'merge_chat_histories':
        return { fill: get('--color-secondary-500') ?? '#64748b', stroke: get('--color-secondary-800') ?? '#1e293b' };
      case 'append_string_to_chat_history':
        return { fill: get('--color-accent-400') ?? '#a3a3a3', stroke: get('--color-accent-700') ?? '#404040' };
      default:
