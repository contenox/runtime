...
func (p *GitHubCommentProcessor) ProcessJob(ctx context.Context, job *store.Job) (err error) {
	...
	subjectID := FormatSubjectID(payload.RepoID, payload.PR)
	...
	msg := fmt.Sprintf("🤖 %s\n\n---\n_Generated by contenox_", lastMsg.Content)
	if err = p.githubSvc.PostComment(ctx, payload.RepoID, payload.PR, msg); err != nil {
