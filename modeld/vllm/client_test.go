...
func TestUnit_VLLMProvider_DoesNotInferThinkingFromModelName(t *testing.T) {
	provider := NewVLLMProvider("Qwen/Qwen3-32B", []string{"http://localhost:8000"}, nil, modeld.CapabilityConfig{}, "", nil)
	require.False(t, provider.CanThink(), "model name alone must not set CanThink")

	provider = NewVLLMProvider("custom", []string{"http://localhost:8000"}, nil, modeld.CapabilityConfig{CanThink: true}, "", nil)
...
func TestUnit_BuildChatRequest_MapsThinkingLevels(t *testing.T) {
	...
	req := buildChatRequest("model", []modeld.Message{{Role: "user", Content: "hi"}}, []modeld.ChatArgument{
		modeld.WithTool(tool),
		modeld.WithThink("xhigh"),
	})
...
func TestUnit_VLLMChat_AllowsToolCallsFinishReason(t *testing.T) {
	...
	resp, err := client.Chat(context.Background(), []modeld.Message{{Role: "user", Content: "weather?"}})
