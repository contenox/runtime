...
> Status: product/API blueprint. Scope is the Contenox runtime server and Beam
> UI surface for local inference observability and safe slot control. `modeld`
> remains the native inference daemon and hardware owner.
...
- Do not make Beam a direct daemon client.
- Do not expose arbitrary local model paths from the browser.
- Do not put decode/session controls in Beam.
- Do not make model variants a hidden mutation of base model identity.
- Do not treat LoRA as a safety control.

## Punchline

Beam is the human-facing control surface for the local AI node. `modeld` is the
machine-facing execution engine. The more `modeld` exposes its hardware,
capacity, residency, and variant identity insight through `contenox serve`, the
more local inference becomes understandable and trustworthy.
