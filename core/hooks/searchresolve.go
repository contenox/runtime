...
func (s *SearchResolve) Exec(ctx context.Context, startTime time.Time, input any, dataType taskengine.DataType, transition string, args *taskengine.HookCall) (any, taskengine.DataType, string, error) {
	if dataType != taskengine.DataTypeSearchResults {
		return nil, dataType, transition, fmt.Errorf("unsupported data type: %v", dataType)
	}
	in, ok := input.([]taskengine.SearchResult)
	if !ok {
		return nil, dataType, transition, fmt.Errorf("SERVER BUG: invalid input type")
	}
	if len(in) == 0 {
		return nil, dataType, transition, fmt.Errorf("no results found")
	}
	...
	if distance, ok := args.Args["distance"]; ok {
		...
		if err != nil {
			return nil, dataType, transition, fmt.Errorf("invalid distance: %v", err)
		}
		a := float32(conv)
		distanceF = &a
	}
	...
	if positionArg, ok := args.Args["position"]; ok {
		...
		if err != nil {
			return nil, dataType, transition, fmt.Errorf("invalid position: %v", err)
		}
		position = int(a)
	}
	if position >= len(in) {
		return nil, dataType, transition, fmt.Errorf("position out of range")
	}
	if distanceF != nil && in[position].Distance > *distanceF {
		return nil, dataType, "", fmt.Errorf("distance too large")
	}

	file, err := storeInstance.GetFileByID(ctx, in[position].ID)
	if err != nil {
		return nil, dataType, transition, fmt.Errorf("failed to get file: %v", err)
	}
	blob, err := storeInstance.GetBlobByID(ctx, file.BlobsID)
	if err != nil {
		return nil, dataType, transition, fmt.Errorf("failed to get blob: %v", err)
	}
	if file.Type == "application/json" {
		return blob.Data, taskengine.DataTypeJSON, "application/json", nil
	}
	if file.Type == "text/plain" || file.Type == "text/plain; charset=utf-8" {
		return blob.Data, taskengine.DataTypeString, "text/plain", nil
	}

	return blob.Data, taskengine.DataTypeAny, "any", nil
}
