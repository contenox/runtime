...
// returned version. hash's client hard-fails unless the agent echoes back the
// literal version it sent (tmp/hash acp.go:562), which is stricter than the
// spec and will break interop with any future peer that legitimately answers a
// different version it can still speak. Pinning min-of-both here gives any
// future libacp client consumer the resilient semantics as a reusable
// primitive so a refactor cannot drift toward that fragility.
