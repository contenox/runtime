...
**Status:** vision. Predates the current auth model (loopback-default serving with bearer `TOKEN` protection in `runtime/serverapi/local_security.go`); revisit against that baseline.
