...
func TestSystem_OpenVINOChatContext_ClassifierWarmsChatPath(t *testing.T) {
	...
	withQuestion := func(question string) []modelrepo.Message {
	...
	_, err = warm.Chat(context.Background(), []modelrepo.Message{{Role: "user", Content: "hello"}}, modelrepo.WithMaxTokens(1), modelrepo.WithTemperature(0))
	require.NoError(t, err)
	_, err = cold.Chat(context.Background(), []modelrepo.Message{{Role: "user", Content: "hello"}}, modelrepo.WithMaxTokens(1), modelrepo.WithTemperature(0))
	require.NoError(t, err)

	start := time.Now()
	_, err = warm.Chat(context.Background(), turnA, modelrepo.WithMaxTokens(8), modelrepo.WithTemperature(0), modelrepo.WithSeed(1))
	require.NoError(t, err)
	coldPrefix := time.Since(start)

	start = time.Now()
	warmB, err := warm.Chat(context.Background(), turnB, modelrepo.WithMaxTokens(8), modelrepo.WithTemperature(0), modelrepo.WithSeed(1))
	require.NoError(t, err)
	warmSuffix := time.Since(start)

	start = time.Now()
	coldB, err := cold.Chat(context.Background(), turnB, modelrepo.WithMaxTokens(8), modelrepo.WithTemperature(0), modelrepo.WithSeed(1))
	require.NoError(t, err)
