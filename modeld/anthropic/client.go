...
func applyAnthropicThinking(req *msgcodec.Request, modelName string, cfg *modeld.ChatConfig) {
...
func (c *anthropicChatClient) Chat(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (modeld.ChatResult, error) {
	...
	req.MaxTokens, _ = modeld.ClampMaxOutputTokens(req.MaxTokens, c.maxOutputTokens)
	if c.canThink {
...
func (c *anthropicStreamClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
	cfg := chatConfigFromArgs(args)
	req, nameMap := msgcodec.Build(messages, cfg)
	...
	go func() {
		...
		if err := sc.Err(); err != nil && err != io.EOF {
			reportErr(err)
			select {
			case parcels <- &modeld.StreamParcel{Error: fmt.Errorf("anthropic: stream read: %w", err)}:
			case <-ctx.Done():
			}
			return
		}
...
func (c *anthropicPromptClient) Prompt(ctx context.Context, systemInstruction string, temperature float32, prompt string) (string, error) {
	...
	var chatArgs []modeld.ChatArgument
	if !anthropicStripsTemperatureParams(c.modelName) {
		chatArgs = append(chatArgs, modeld.WithTemperature(float64(temperature)))
	}
...
var (
	_ modeld.LLMChatClient       = (*anthropicChatClient)(nil)
	_ modeld.LLMStreamClient     = (*anthropicStreamClient)(nil)
	_ modeld.LLMPromptExecClient = (*anthropicPromptClient)(nil)
)
