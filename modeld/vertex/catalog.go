...
func init() {
	modeld.RegisterCatalogProvider("vertex-google", newGoogleCatalog)
}
...
func newGoogleCatalog(spec modeld.BackendSpec, opts modeld.CatalogOptions) (modeld.CatalogProvider, error) {
	if spec.BaseURL == "" {
...
func (p *googleCatalogProvider) ListModels(ctx context.Context) ([]modeld.ObservedModel, error) {
	return p.listGoogleModelsFromVertexPublisher(ctx)
}

func (p *googleCatalogProvider) listGoogleModelsFromVertexPublisher(ctx context.Context) ([]modeld.ObservedModel, error) {
	tokenFn := p.tokenFn
	if tokenFn == nil {
	...
	out := make([]modeld.ObservedModel, 0, len(names))
...
func enrichGooglePublisherModel(name string) modeld.ObservedModel {
	n := strings.ToLower(name)
	om := modeld.ObservedModel{Name: name}
	switch {
...
func (p *googleCatalogProvider) ProviderFor(model modeld.ObservedModel) modeld.Provider {
