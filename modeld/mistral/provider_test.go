...
func TestUnit_MistralChat_RequestShapeAndResponse(t *testing.T) {
	...
	p := NewMistralProvider("secret-key", "mistral-large-latest", []string{srv.URL},
		modeld.CapabilityConfig{CanChat: true, MaxOutputTokens: 64}, srv.Client(), nil)
	chat, err := p.GetChatConnection(context.Background(), "")
	require.NoError(t, err)

	res, err := chat.Chat(context.Background(), []modeld.Message{{Role: "user", Content: "hi"}}, modeld.WithMaxTokens(128))
	require.NoError(t, err)
...
func TestUnit_MistralCatalog_DoesNotInferThinkingFromModelName(t *testing.T) {
	...
	catalog, err := modeld.NewCatalogProvider(modeld.BackendSpec{Type: "mistral", BaseURL: srv.URL})
	require.NoError(t, err)
...
func TestUnit_MistralProvider_CanThinkFromCapabilityConfigOnly(t *testing.T) {
	provider := NewMistralProvider("", "magistral-medium-latest", []string{"http://localhost"}, modeld.CapabilityConfig{}, nil, nil)
	require.False(t, provider.CanThink(), "model name alone must not set CanThink")

	provider = NewMistralProvider("", "custom", []string{"http://localhost"}, modeld.CapabilityConfig{CanThink: true}, nil, nil)
...
func TestUnit_MistralCatalog_Registered(t *testing.T) {
	cp, err := modeld.NewCatalogProvider(modeld.BackendSpec{Type: "mistral", APIKey: "k"})
	...
	prov := cp.ProviderFor(modeld.ObservedModel{
		...
		CapabilityConfig: modeld.CapabilityConfig{CanChat: true},
	})
