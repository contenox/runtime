...
func Build(messages []modeld.Message, cfg *modeld.ChatConfig) (Request, map[string]string) {
...
func DecodeResponse(raw []byte, nameMap map[string]string) (modeld.ChatResult, error) {
	...
	if err := json.Unmarshal(raw, &resp); err != nil {
		return modeld.ChatResult{}, fmt.Errorf("messages: decode response: %w", err)
	}
	if resp.StopReason == "refusal" {
		return modeld.ChatResult{}, modeld.ErrRefused
	}
	if len(resp.Content) == 0 {
		return modeld.ChatResult{}, fmt.Errorf("messages: empty content (stop_reason=%s)", resp.StopReason)
	}
...
func newToolCall(id, name, args string) modeld.ToolCall {
	tc := modeld.ToolCall{ID: id, Type: "function"}
...
func (d *StreamDecoder) DecodeLine(payload []byte) (*modeld.StreamParcel, error) {
	...
	case "content_block_delta":
		...
		case "text_delta":
			if ev.Delta.Text != "" {
				return &modeld.StreamParcel{Data: ev.Delta.Text}, nil
			}
		case "thinking_delta":
			if ev.Delta.Thinking != "" {
				return &modeld.StreamParcel{Thinking: ev.Delta.Thinking}, nil
			}
