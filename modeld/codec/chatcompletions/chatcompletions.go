...
func Build(model string, messages []modeld.Message, cfg *modeld.ChatConfig) (Request, map[string]string) {
...
func DecodeResponse(raw []byte, nameMap map[string]string) (modeld.ChatResult, error) {
	var resp Response
	if err := json.Unmarshal(raw, &resp); err != nil {
		return modeld.ChatResult{}, fmt.Errorf("chatcompletions: decode response: %w", err)
	}
	if len(resp.Choices) == 0 {
		return modeld.ChatResult{}, fmt.Errorf("chatcompletions: no choices in response")
	}
	choice := resp.Choices[0]
	if choice.Message.Content == "" && len(choice.Message.ToolCalls) == 0 && choice.Message.ReasoningContent == "" {
		return modeld.ChatResult{}, fmt.Errorf("chatcompletions: empty content (finish_reason=%s)", choice.FinishReason)
	}
...
func decodeToolCalls(in []wireToolCall, nameMap map[string]string) []modeld.ToolCall {
	var out []modeld.ToolCall
	for _, tc := range in {
		name := tc.Function.Name
...
// struct on modeld.ToolCall, so it is constructed via this helper.
func newToolCall(id, typ, name, args string) modeld.ToolCall {
	tc := modeld.ToolCall{ID: id, Type: typ}
...
func (d *StreamDecoder) DecodeLine(payload []byte) (*modeld.StreamParcel, error) {
	...
	if delta.Content != "" || delta.ReasoningContent != "" {
		return &modeld.StreamParcel{Data: delta.Content, Thinking: delta.ReasoningContent}, nil
	}
	return nil, nil
}
...
func (d *StreamDecoder) ToolCalls() []modeld.ToolCall {
	if d.maxIndex < 0 {
		return nil
	}
	var out []modeld.ToolCall
