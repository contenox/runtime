...
> **Status:** historical record — roadmap notes from the modeld refactor; build/test recipes here predate the current Makefile targets (see `CONTRIBUTING.md`).
...
- **VLLMParserWrapper** — formalize: either a Python parser-object bridge or keep the
  explicit native-bridge error, documented (per `modeld/openvino/s2-7-protocol-registry.md`).
