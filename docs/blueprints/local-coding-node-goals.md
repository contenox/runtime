...
> "what" behind the implementation tracks: `plan-openvino.md`,
> `plan-llamacpp.md`, and the AI PC compatibility lane in
> `plan-ortgenai-windows-ai.md`. Backends are *means* to these goals, not the
> goals.
...
- **Budget-HARDWARE latency goal NOT yet proven:** everything ran CPU-only, where
  ...
  number, then the Tier targets on the budget node, is what the `plan-llamacpp.md`
  track now drives.
