...
> **Sibling docs:** `modeld-interface-boundary.md` (the compute boundary),
> `local-runtime-owner-coordination.md` (the owner lease), `plan-llamacpp.md`.
...
CGO `modeld` binary (see `modeld-interface-boundary.md`). The runtime must
