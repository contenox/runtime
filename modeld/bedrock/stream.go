...
// Stream implements modeld.LLMStreamClient via Bedrock ConverseStream. The
...
func (c *bedrockStreamClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
	...
	go func() {
		...
		for ev := range stream.Events() {
			...
			chunkCount++
			select {
			case parcels <- &modeld.StreamParcel{Data: td.Value}:
			case <-ctx.Done():
				return
			}
		}
		if err := stream.Err(); err != nil {
			reportErr(err)
			select {
			case parcels <- &modeld.StreamParcel{Error: fmt.Errorf("bedrock stream: %w", err)}:
			case <-ctx.Done():
			}
			return
		}
...
var _ modeld.LLMStreamClient = (*bedrockStreamClient)(nil)
