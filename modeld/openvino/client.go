...
func (c *genAIClient) Chat(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (modeld.ChatResult, error) {
	...
	if len(cfg.Tools) > 0 && strings.TrimSpace(c.toolProtocol) == "" {
		return modeld.ChatResult{}, errors.New("openvino model profile does not declare a tool_calls.protocol")
	}
	...
	return modeld.ChatResult{
		Message:   modeld.Message{Role: "assistant", Content: visible, Thinking: thinking, ToolCalls: toolCalls},
		ToolCalls: toolCalls,
	}, nil
}

func (c *genAIClient) Prompt(ctx context.Context, systemInstruction string, temperature float32, prompt string) (string, error) {
	messages := []modeld.Message{{Role: "user", Content: prompt}}
...
func (c *genAIClient) Stream(ctx context.Context, messages []modeld.Message, args ...modeld.ChatArgument) (<-chan *modeld.StreamParcel, error) {
	cfg := &modeld.ChatConfig{}
	for _, a := range args {
		a.Apply(cfg)
	}
	...
	out := make(chan *modeld.StreamParcel, 16)
	go func() {
		defer close(out)
		for chunk := range chunks {
			if chunk.Error != nil {
				out <- &modeld.StreamParcel{Error: chunk.Error}
				return
			}
			if chunk.Text == "" {
				continue
			}
			out <- &modeld.StreamParcel{Data: chunk.Text}
		}
	}()
	return out, nil
}
...
func generateOptions(cfg *modeld.ChatConfig, maxOutputTokens int) ovsession.GenerateOptions {
	maxTokens := defaultPromptMaxTokens(maxOutputTokens)
	if cfg != nil && cfg.MaxTokens != nil && *cfg.MaxTokens > 0 {
		maxTokens = *cfg.MaxTokens
	}
	maxTokens, _ = modeld.ClampMaxOutputTokens(maxTokens, maxOutputTokens)
...
func defaultPromptMaxTokens(maxOutputTokens int) int {
	maxTokens, _ := modeld.ClampMaxOutputTokens(defaultMaxNewTokens, maxOutputTokens)
	return maxTokens
}

func toChatMessages(messages []modeld.Message) []ovsession.ChatMessage {
...
func (c *genAIClient) renderPrompt(messages []modeld.Message, toolsJSON string) string {
...
func buildPrompt(messages []modeld.Message) string {
...
var (
	_ modeld.LLMChatClient       = (*genAIClient)(nil)
	_ modeld.LLMPromptExecClient = (*genAIClient)(nil)
	_ modeld.LLMStreamClient     = (*genAIClient)(nil)
)
